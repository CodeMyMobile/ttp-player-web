...
- `COACH_API_URL`: optional full HTTPS URL overriding the default public coach endpoint.

The build stops when the endpoint fails, the response is not an array, a coach has no stored `slug`, or the public roster is empty. `VITE_GOOGLE_API_KEY` is not used here; courts are mapped solely from the committed `src/data/venues.json` allowlist.
