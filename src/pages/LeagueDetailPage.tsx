...
/**
 * A league match need that another player has accepted — i.e. a time and place two
 * players have agreed on. This is scheduling only: league fixtures are generated by
 * admin and scoring runs separately through the fixture-based log-result flow, so an
 * accepted need never becomes a `league_matches` row and cannot be read from
 * /leagues/{id}/fixtures.
 *
 * Before this tab existed the accepted match had nowhere to appear at all. Every
 * match-need listing filters to status "open" (routes/leagues.js myNeeds, and
 * league_match_needs.js for the browse and suggestion lists), so the moment the accept
 * flipped the row to "confirmed" it dropped out of all of them — the player saw the
 * card vanish and nothing take its place.
 *
 * The rows are read from the general matches list instead, which does not exclude
 * league matches and enriches each one with its participants' profiles. Both players
 * are participants: the host is added as "hosting" when the need is posted and the
 * accepter as "confirmed" on accept, so this returns the match for either side.
 */
type ScheduledMatch = {
  id: number | string;
...
const toScheduledMatch = (raw: unknown, viewerId: unknown): ScheduledMatch | null => {
  ...
  const participants = Array.isArray(match.participants) ? (match.participants as RawParticipant[]) : [];
  // The opponent is whoever is not the viewer. Falling back to the host lets the row
  // still name someone when the viewer is missing from the participant list.
  const opponent =
...
const LeagueDetailPage = () => {
  ...
  // endpoint — see the ScheduledMatch note above for why. Kept out of the main
  ...
  const loadScheduled = useMemo(
    () => async (signal?: AbortSignal) => {
      ...
      try {
        ...
        const rows = matches
          .map((match) => toScheduledMatch(match, userId))
          .filter((match): match is ScheduledMatch => match !== null)
          // filter=my spans every league the player is in, so scope to this one.
          .filter((match) => String(match.leagueId ?? "") === String(id))
          .sort((a, b) => {
